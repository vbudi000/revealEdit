...
// end convert_to_v8 block
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method createFromStream has thrown an error.")->ToObject();
...
// end convert_to_v8 block
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method createFromstreamCommit has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        Local<v8::Value> argv[1] = {
          err
        };
