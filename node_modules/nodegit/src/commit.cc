...
// end convert_from_v8 block
  ...
  Nan::Callback *callback = new Nan::Callback(Local<Function>::Cast(info[9]));
...
// end convert_from_v8 block
  baton->repo = from_repo;
// start convert_from_v8 block
  const char * from_commit_content;
...
// start convert_from_v8 block
  const char * from_signature_field;
...
// end convert_to_v8 block
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method createWithSignature has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        Local<v8::Value> argv[1] = {
          err
        };
