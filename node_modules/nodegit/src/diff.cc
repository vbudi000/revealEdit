...
{
  BlobToBuffer_FileCbBaton* baton =
    new BlobToBuffer_FileCbBaton(1);

  baton->delta = delta;
  baton->progress = progress;
  baton->payload = payload;

  return baton->ExecuteAsync((uv_async_cb) BlobToBuffer_file_cb_async);
}
...
{
  BlobToBuffer_BinaryCbBaton* baton =
    new BlobToBuffer_BinaryCbBaton(0);

  baton->delta = delta;
  baton->binary = binary;
  baton->payload = payload;

  return baton->ExecuteAsync((uv_async_cb) BlobToBuffer_binary_cb_async);
}
...
{
  BlobToBuffer_HunkCbBaton* baton =
    new BlobToBuffer_HunkCbBaton(1);

  baton->delta = delta;
  baton->hunk = hunk;
  baton->payload = payload;

  return baton->ExecuteAsync((uv_async_cb) BlobToBuffer_hunk_cb_async);
}
...
{
  BlobToBuffer_LineCbBaton* baton =
    new BlobToBuffer_LineCbBaton(1);
  ...
  return baton->ExecuteAsync((uv_async_cb) BlobToBuffer_line_cb_async);
}
...
// end convert_to_v8 block
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method indexToIndex has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        Local<v8::Value> argv[1] = {
          err
        };
...
void GitDiff::MergeWorker::HandleOKCallback()
{
  if (baton->error_code == GIT_OK) {
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method merge has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        Local<v8::Value> argv[1] = {
          err
        };
