  ...
  const char * refname,     const char * remote_url,     const git_oid * oid,     unsigned int is_merge,     void * payload  )
{
  FetchheadForeach_CallbackBaton* baton =
    new FetchheadForeach_CallbackBaton(0);
  ...
  return baton->ExecuteAsync((uv_async_cb) FetchheadForeach_callback_async);
}
...
{
  MergeheadForeach_CallbackBaton* baton =
    new MergeheadForeach_CallbackBaton(0);

  baton->oid = oid;
  baton->payload = payload;

  return baton->ExecuteAsync((uv_async_cb) MergeheadForeach_callback_async);
}
